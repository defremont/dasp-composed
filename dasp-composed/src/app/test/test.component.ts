...
@Component({
    ...
    styleUrls: ["./test.component.scss".toString()]
})
export class TestComponent implements OnInit, OnDestroy {
            ...
            .then(() => {
                ...
                return this.clientService
                    ...
                    .then(historianRegistry => {
                        ...
                        let resource = this.registries[
                            "participants"
                        ][0].getAll();
                        this.registries["participants"][0]
                            .getAll()
                            .then(resources => {
                                ...
                                this.actualId = resource[0].getIdentifier();
                                this.loadCount();
                            })
                            .catch(error => {
    ...
    loadCount() {
        this.registries["assets"][2].getAll().then(resources => {
            ...
            resources.forEach(resource => {
                if (
                    resource.reviewer.getIdentifier() === this.actualId &&
                    (!resource.acc || !resource.complete)
                ) {
                    this.revCount++;
                }
            });
   ...
   `
        ...
        if (this.transactionTypes && this.transactionTypes.length > 0) {
            for (let transactions in this.transactionTypes) {
                if (this.transactionTypes[transactions]["name"] === value) {
                    this.selectedTransaction = this.transactionTypes[
                        transactions
                    ];
            ...
            this.hiddenTransactionItems.set(
                this.selectedTransaction.getIdentifierFieldName(),
                uuid.v4()
            );
    ...
    setChosenMenu(chosenMenu) {
        ...
        this.chosenMenu === "toReview"
            ? (this.chosenRegistry = this.registries["assets"][2])
            : null;
        this.chosenMenu === "myArticles"
            ? (this.chosenRegistry = this.registries["assets"][0])
            : null;
        this.chosenMenu === "publicArticles"
            ? (this.chosenRegistry = this.registries["assets"][0])
            : null;
        this.chosenMenu === "myArticleRevisions"
            ? (this.chosenRegistry = this.registries["assets"][2])
            : null;
        this.chosenMenu === "publicRevisions"
            ? (this.chosenRegistry = this.registries["assets"][2])
            : null;
        this.chosenMenu === "reviewed"
            ? (this.chosenRegistry = this.registries["assets"][2])
            : null;
    }
    loadTransaction() {
        let introspector = this.clientService
            .getBusinessNetwork()
            .getIntrospector();
        this.transactionTypes = introspector
    ...
    async createRevision() {
        ...
        let resource = serializer.fromJSON({
            ...
            article:
                "resource:org.dasp.net.Article#" + this.id.replace(/"/g, "")
        });
    ...
    private async submitSpecTransaction() {
        ...
        await ipfs
            ...
            .then(() => {
                ...
                return this.clientService
                    .getBusinessNetworkConnection()
                    .submitTransaction(this.submittedTransaction);
            })
            .then(() => {
                this.definitionError = null;
                this.id = JSON.stringify(
                    this.submittedTransaction["transactionId"]
                ).replace(/"/g, "");
                this.createRevision();
