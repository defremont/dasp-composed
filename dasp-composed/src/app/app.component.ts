...
export class AppComponent implements OnInit, OnDestroy {
    ...
    queryParamsUpdated(queryParams: Object): Promise<any> {
        ...
        return this.initializationService.initialize().then(() => {
    ...
    onBusyStatus(busyStatus) {
        ...
        if (busyStatus && !busyStatus.force) {
            ...
            if (
                !card ||
                (card && card.getConnectionProfile()["x-type"] === "web")
            ) {
    ...
    onTransactionEvent(eventStatus) {
        if (eventStatus) {
            ...
            transactionModalRef.componentInstance.transaction =
                eventStatus.transaction;
